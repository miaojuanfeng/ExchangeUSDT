...
//		OrderTimeout.getInstance().setOrderService(ordersService);
		...
		//回调管理。
		Turntable.getInstance().init(merchantNotifyService, notifyCommercial,ordersService,paymentRotateService);		
		// 过期订单处理 。
		new CheckOrderTimeout(ordersService,merchantNotifyService,notifyCommercial,paymentRotateService).CheckOrder();
