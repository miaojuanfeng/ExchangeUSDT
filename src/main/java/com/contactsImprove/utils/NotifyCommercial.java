...
public class NotifyCommercial {
	...
	public byte notifyOrderFinish(MerchantNotify orderPara,String privateKey) {	
		...
		paramap.put("actualAmount", np.getActualAmount()+"");
		paramap.put("actualCurrencyAmount", np.getActualCurrencyAmount()==null ? "" : np.getActualCurrencyAmount()+"");
		paramap.put("merchantRate", np.getMerchantRate()==null ? "" : np.getMerchantRate()+"");
		paramap.put("poundage",  np.getPoundage()==null ? "" : np.getPoundage()+"");
	...
	public MerchantNotify createNotify(Orders order,String tradeStatus) {
	    ...
	    np.setActualAmount(order.getActualAmount());
	    np.setActualCurrencyAmount(order.getActualCurrencyAmount());
	    np.setPoundage(order.getPoundage());
	    np.setMerchantRate(order.getMerchantRate());
