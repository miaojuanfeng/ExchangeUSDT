...
/*		Users user=new Users();
	...
	@Override
	public BigDecimal userIncomeMoney(Users user, String tradeNumber, BigDecimal amount, Date date) {
		...
		if( user != null ) {
			UserWallet userWallet = userWalletDao.findByLock(user.getUserId(), SystemConst.USDT);
			...
			userJA.setCurrencyNumberBefore(userWallet.getCurrencyNumber());
			userJA.setCurrencyNumberAfter(userWallet.getCurrencyNumber().add(userIncome));
			userJA.setFreezeNumberBefore(userWallet.getFreezeNumber());
			userJA.setFreezeNumberAfter(userWallet.getFreezeNumber());
			userJA.setCreateTime(date);
			userJA.setType(JournalConst.JournalType.income.status);
			userJA.setCurrencyType(JournalConst.CurrencyType.USDT.status);
			userJA.setRemark("币商提成");
			journalAccounts.add(userJA);
			
			UserWallet updateUserWallet = new UserWallet();
			updateUserWallet.setWalletId(userWallet.getWalletId());
			updateUserWallet.setCurrencyNumber(userWallet.getCurrencyNumber().add(userIncome));
			userWallets.add(updateUserWallet);
			
			totalMoney = totalMoney.add(userIncome);
		}
		...
		if( users.size() > 0 ) {
			...
			UserWallet userWallet = userWalletDao.findByLock(fUser.getUserId(), SystemConst.USDT);
			...
			userJA.setCurrencyNumberBefore(userWallet.getCurrencyNumber());
			userJA.setCurrencyNumberAfter(userWallet.getCurrencyNumber().add(money));
			userJA.setFreezeNumberBefore(userWallet.getFreezeNumber());
			userJA.setFreezeNumberAfter(userWallet.getFreezeNumber());
			userJA.setCreateTime(date);
			userJA.setType(JournalConst.JournalType.income.status);
			...
			UserWallet updateUserWallet = new UserWallet();
			updateUserWallet.setWalletId(userWallet.getWalletId());
			updateUserWallet.setCurrencyNumber(userWallet.getCurrencyNumber().add(money));
			userWallets.add(updateUserWallet);
		...
		if( shareholders.size() > 0 ) {
			money = userIncome.multiply(SystemConst.income_shareholder_money_rate.divide(new BigDecimal(shareholders.size()).setScale(SystemConst.decimal_place_num, BigDecimal.ROUND_HALF_UP))).setScale(SystemConst.decimal_place_num, BigDecimal.ROUND_HALF_UP);
		}
		for (Users shareholder : shareholders) {
			UserWallet userWallet = userWalletDao.findByLock(shareholder.getUserId(), SystemConst.USDT);
